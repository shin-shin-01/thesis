...
\subsection{文字の縦横比変更}
1つの単語における，すべての点のy座標（もしくはx座標）の平均値を取り，すべての点のy座標（もしくはx座標）に対しーーーーーーーーーーーーー\textbf{図}に文字の縦横比変更の原理を示す．
文字の拡張比率を$a$，1つの単語上のすべての点の$y$座標の平均を$\bar{x}$とし，同単語上の任意の点の座標を$(x, y)$としたとき，
その点の$x$座標が$\bar{y}$より大きいときに$y$方向にーーーーーー移動させた後の座標$(X, Y)$は 式~\ref{eq:yratio}で表される．
